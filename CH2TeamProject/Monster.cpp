...
{
	int damage = this->power - character->getDefence();
	if (damage <= 0)
	{
		damage = 1;
	}
	...
	cout << "플레이어의 방어: " << character->getDefence() << endl;
	cout << "* 몬스터가 플레이어 (" << character->getNickname() << ")에게 " << damage << "대미지를 입힙니다!" << endl;
	...
	{
		cout << "현재 HP: " << character->getHP() << endl;
	}
}
